...
class pixyTuning(object):
    ...
    # output        - "calibrationCurve_X.csv" with target tracking error in ([deg]), camera angle in ([deg]) and the presence of target at each frame time
    # fVideo        - frequency of stimulus in MATLAB generated video file (>=0)
    # predictFnc    - target tracking prediction function written by you (function)
    # startAngle    - (optional, default 35) initial camera angle in ([deg])
    def visualPrediction(self, fVideo, predictFnc, startAngle=35):
        ...
        errors = []
        angles = []
        lostFlag = [] # binary of whether the camera sees the target or not
        ...
        # look in the general direction of the target start point
        self.bot.setServoPosition(self.startAngle)
        ...
        while time() < tEnd:
            ...
            while self.cam.isBiggestSig(self.targetID) and time() < tEnd:
                error, servoNewPos = self.updateServo()
            ...
            # once the block disappears, invoke the function supplied by you!
            while not self.cam.isBiggestSig(self.targetID) and time() < tEnd:

                # update time since losing target
                tOcc = time() - tStart
                times.append(time() - tStart)
                errors.append(error)
                angles.append(servoNewPos)
                lostFlag.append(1)
                try:
                    # move to the position demanded by your prediction fucntion
                    pos = predictFnc(tOcc, times, errors, angles)
                    servoNewPos = self.bot.setServoPosition(pos)
                except:
        ...
        # write to csv file
        fileName = "calibrationCurve_occluded" + str(fVideo) + "Hz_" + str(fileNumber) + ".csv"
        while os.path.exists(fileName):
            fileNumber = fileNumber + 1
            fileName = "calibrationCurve_occluded" + str(fVideo) + "Hz_" + str(fileNumber) + ".csv"
        with open(fileName, "w+") as myCsv:
            csvWriter = csv.writer(myCsv, delimiter=',')
            csvWriter.writerows([times, errors, angles, lostFlag]) # 4 row output
